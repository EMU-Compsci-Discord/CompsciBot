...
class ChannelManager(commands.Cog, name="channelmanager"):
    ...
    async def deleteAll(self, context):
        """
        Deletes all channels in Test Discord that aren't in TextChanne's and VoiceChannels category
        """
        if context.guild.id == testGuild:  # WILL NOT RUN IN COMPSCI
            for channel in context.guild.channels:
                if channel.category.id != 931309621770080388 or channel.category.id != 931309621770080389:
                    channel.delete()
